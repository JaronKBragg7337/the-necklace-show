...
function makeWater(o={}){
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader:[
      ...
      'uniform float uGlowStr,uSparkle,uTime,uFogN,uFogF;',
      'varying vec3 vN; varying vec3 vW;',
      'void main(){',
...
function env(scene, o={}){
  ...
  if(o.dir){ const d = new THREE.DirectionalLight(o.dirColor??0xbfd4ff, o.dirI??0.7);
    d.position.set(...(o.dir)); scene.add(d); }
  return scene;
}
...
function figure(o={}){
  ...
  const g = new THREE.Group();
  if(o.robe){
    const robe = new THREE.Mesh(new THREE.CylinderGeometry(0.13,0.36,h*0.78,7), std(o.robe));
    robe.position.y=h*0.39; g.add(robe);
  } else {
    const body = new THREE.Mesh(new THREE.CylinderGeometry(0.15,0.19,h*0.55,7), std(o.color??0x5a4632));
    body.position.y=h*0.52; g.add(body);
    const legs = new THREE.Mesh(new THREE.CylinderGeometry(0.13,0.15,h*0.3,7), std(0x2c2620));
    legs.position.y=h*0.15; g.add(legs);
  }
  const head = new THREE.Mesh(new THREE.SphereGeometry(h*0.085,10,8), std(o.skin??0xc9a486));
  head.position.y=h*0.9; g.add(head);
  if(o.staff){
    const st = new THREE.Mesh(new THREE.CylinderGeometry(0.025,0.035,h*1.05,6), std(0x3a2c1c));
    st.position.set(0.26,h*0.52,0); g.add(st);
    const cr = new THREE.Mesh(new THREE.OctahedronGeometry(0.1), basic(0x9fe8ff));
    cr.position.set(0.26,h*1.08,0); g.add(cr);
...
function buildS1(world){
  ...
  const f1 = figure({color:0x4a3a2c, h:1.7}); f1.position.set(-1.2,0.75,3.9); f1.scale.y=0.72;
...
function buildS6(world){
  ...
  const watcher = figure({color:0x1c202a, h:1.75, skin:0xb8a898});
...
function buildS7(world){
  ...
  const thorne = figure({color:0x1a1c24, h:1.8, skin:0xc8b8a8}); thorne.position.set(0,0,-3.6);
...
function buildS8(world){
  ...
  const jalen = figure({color:0x4a3a2c, h:1.78}); jalen.position.set(0,0,1.3); jalen.rotation.y=Math.PI; scene.add(jalen);
  const thorne = figure({color:0x1c1e26, h:1.8, skin:0xc8b8a8}); thorne.position.set(0.2,0,-1.7); scene.add(thorne);
  ...
  const maya = figure({color:0x6e5a8a, h:1.5, skin:0xb58a6a}); maya.position.set(1.2,0,0.6); scene.add(maya);
  const leo = figure({color:0x3a5a7a, h:1.05, skin:0xb58a6a}); leo.position.set(1.7,0,0.9); scene.add(leo);
...
function buildS10(world){
  ...
  const naia = figure({robe:0x2c4a5e, h:2.1, skin:0x8a6a52, staff:true});
...
function buildS11(world){
  ...
  const jalen = figure({color:0x5a4a38, h:1.78}); jalen.position.set(-1.0,0.56,2.2); jalen.rotation.y=2.4; scene.add(jalen);
  const maya = figure({color:0x6e5a8a, h:1.5, skin:0xb58a6a}); maya.position.set(-2.2,0.56,3.0); maya.rotation.y=2.2; scene.add(maya);
  const leo = figure({color:0x3a5a7a, h:1.05, skin:0xb58a6a}); leo.position.set(-2.9,0.56,2.3); leo.rotation.y=2.3; scene.add(leo);
...
function buildS14(world){
  ...
  // figures
  const thorne = figure({color:0x1c1e26, h:1.8, skin:0xc8b8a8}); thorne.position.set(2.8,0,2.2); thorne.lookAt(0,2.6,0); scene.add(thorne);
...
function buildS15(world){
  ...
  scene.add(starField(400));
  const water = makeWater({deep:0x0a1420, shallow:0x1c3c50, amp:.5, freq:.1, speed:1.6,
...
{ ch:'Chapter Three', title:'The Weight of What Was', dur:28, build:buildS3,
...
{ ch:'Chapter Four', title:'Whispers in the Static', dur:25, build:buildS4,
...
{ ch:'Chapter Five', title:'Whispers of Ancient Wood', dur:28, build:buildS5,
...
{ ch:'Chapter Seven', title:'The Unveiling', dur:26, build:buildS7,
...
{ ch:'Chapter Eleven', title:'Heart of Shifting Sands', dur:29, build:buildS11,
...
{ ch:'Chapter Thirteen', title:'The Echo Between Waves', dur:29, build:buildS13,
...
function applyCamera(){
  const def = SCENES[idx], c = cur.cam;
  const k = smooth(Math.min(1, sceneT/def.dur));
  if(c.mode==='path'){
